...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  Checkout-local Claude overlay (`.claude/settings.local.json`), Spotlight (`.metadata_never_index`), language-server cache (`.serena/`), and local task scratch (`tasks/`) are gitignored for the same reason.

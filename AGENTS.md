...
state/               runtime records and signals; gitignored
  ...
  <id>.copilot-session  copilot busy-source binding (copilot home plus the session id fm-spawn pinned) written by fm-spawn; removed by teardown
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, and `cursor`, plus `muse` and `copilot` for crewmates and scouts only; never dispatch on an unverified adapter.

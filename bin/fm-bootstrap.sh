...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","pi-signed","grok","kimi","cursor","copilot","muse"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      elif ($e | type) != "string" then false
      elif (["claude","pi","pi-signed","muse","copilot"] | index($h)) then (["low","medium","high","xhigh","max"] | index($e))

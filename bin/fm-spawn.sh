...
launch_template() {
  ...
  case "$harness" in
    ...
    cursor) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u COPILOT_CLI -u CURSOR_INVOKED_AS __CURSORBIN__ --trust --yolo __MODELFLAG__--workspace __WORKTREE__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
    ...
    copilot) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u CURSOR_AGENT -u CURSOR_INVOKED_AS COPILOT_HOME=__COPILOTHOME__ __COPILOTBIN__ --yolo --no-ask-user --session-id __COPILOTSESSION__ __MODELFLAG____EFFORTFLAG__-i "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
...
case "$LAUNCH" in
  *__COPILOTBIN__*)
    ...
    LAUNCH=${LAUNCH//__COPILOTHOME__/$(shell_quote "$COPILOT_HOME_DIR")}
    ;;
esac
...
case "$HARNESS" in
  claude|codex|opencode|pi|pi-signed|grok|kimi|muse)
    LAUNCH="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u COPILOT_CLI $LAUNCH"
    ;;
esac

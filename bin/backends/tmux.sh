...
fm_backend_tmux_classify_process_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # copilot is anchored for the same reason muse is: GitHub Copilot CLI is a
    # single compiled executable whose live process name is exactly `copilot`
    # (verified on Copilot CLI 1.0.80, so no bare-`node` reclassification like
    # cursor's is needed), while a *copilot* glob would also own an unrelated
    # command such as a copilot-language-server or a wrapper script.

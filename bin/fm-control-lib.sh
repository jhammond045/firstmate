...
fm_control_harness_supported() {  # <harness>
  case "${1-}" in
    claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|copilot|muse) return 0 ;;
  esac
  return 1
}
...
# Which task kinds an adapter is verified to run. muse and copilot are
# crewmate/scout adapters only: neither has a primary supervision protocol, and
# bin/fm-spawn.sh refuses a --secondmate launch on either. copilot's own hook
# surface (~/.copilot/hooks/*.json) is real but its event vocabulary is
# undocumented in the shipped CLI help, and the only event observed in a
# working installation is sessionStart - nothing that could close a turn - so a
# copilot primary could not run the no-turn-ends-blind guard. The control plane
...
fm_control_harness_supports_kind() {  # <harness> <kind>
  ...
  case "$harness" in
    muse|copilot) [ "$kind" != secondmate ] || return 1 ;;
  esac
  return 0
}

# The key that cancels a running turn. Escape for every adapter except grok and
# copilot. grok's Esc only moves focus to the scrollback. copilot's footer
# advertises `esc interrupt`, but a delivered Escape does NOT cancel: six
# consecutive Escapes left a running turn working (Copilot CLI 1.0.80), during
# both a foreground shell tool call and plain inference, while a single Ctrl+C
# cancelled promptly and left the CLI running. Do not "restore" this to the
# fleet default on the strength of that rendered hint.
...
fm_control_interrupt_repeat() {  # <harness>
  case "${1-}" in
    opencode) printf '2' ;;
    claude|codex|pi|pi-signed|grok|kimi|cursor|copilot|muse) printf '1' ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
    claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|copilot) ;;
    *) return 1 ;;
  esac
}
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    # copilot's own session event log types the cancellation: a Ctrl+C writes
...
fm_control_exit_command() {  # <harness>
  case "${1-}" in
    claude|opencode|grok|kimi|cursor|copilot|muse) printf '/exit' ;;
...
fm_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    # copilot installs no hook either: its busy source is its own session event
    # log, bound to the pane by the session id fm-spawn chose with --session-id
    # and recorded in this one sidecar. A relaunch ONTO copilot rewrites it, and
    # a relaunch AWAY from copilot must retire it so no retired incarnation's
    # session binding outlives the agent.

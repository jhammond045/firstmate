...
#     strict fm_nm_head_matches_worktree; not the current-state unfetched-tip
#     variant crew-state uses) both match this worktree, then runs
...
validate_worktree_teardown_safety() {
  ...
  dirty=$(printf '%s\n' "$dirty_raw" | grep -vE '^\?\? (\.claude/|\.firstmate/|\.fm-(grok|kimi)-turnend$)' | head -1 || true)

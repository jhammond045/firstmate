...
| Interrupt | Single `Ctrl+C`, which cancels the turn and leaves the CLI running with an empty composer, so NO clear key is needed. `Esc` does NOT interrupt despite the footer's `esc interrupt` hint. `bin/fm-control.sh interrupt` reports `cancel=confirmed`, claimed only from a NEW abort naming a user-initiated reason beyond the count of those taken before the key, because the log keeps every earlier interrupt's aborts and types a crash or timeout abort the same way. |
...
Only a concrete model id can be substituted, so `auto` and firstmate's own "pass no `--model`" sentinel are exempt: the exemption is decided before the read, so such a spawn waits for nothing and is told nothing (`fm_busy_copilot_model_is_substitutable` in `bin/fm-busy-lib.sh` owns that rule).

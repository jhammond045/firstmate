...
test_ship_modes_generate_clean_briefs() {
  ...
  for id_mode in "brief-nomistakes-a1:no-mistakes" "brief-directpr-a2:direct-PR" "brief-localonly-a3:local-only"; do
    ...
    assert_grep "created empty when you were spawned" "$brief" \
      "$id: brief did not tell the worker its status file already exists at launch"
...
test_secondmate_directory_paths_are_absolute_and_output_is_stable() {
  ...
  assert_grep '>> .firstmate/status' "$brief" \
    "relative FM_HOME changed the worktree-relative secondmate status path"
  ...
  assert_grep '>> .firstmate/status' "$brief" \
    "relative FM_STATE_OVERRIDE changed the worktree-relative secondmate status path"
  ...
  assert_grep '>> .firstmate/status' "$brief" \
    "relative FM_DATA_OVERRIDE changed the worktree-relative secondmate status path"
  ...
  pass "fm-brief.sh: relative directory inputs ignore CDPATH, render a stable worktree-relative status path, or fail loudly"
}
...
test_scout_and_secondmate_scaffold() {
  ...
  assert_grep "created empty when you were spawned" "$brief" \
    "scout brief did not tell the worker its status file already exists at launch"
  ...
  assert_grep "created empty when you were spawned" "$brief" \
    "secondmate charter did not tell the worker its status file already exists at launch"

...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without write evidence or an attributed live pipeline agent, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
The same wedge branch also defers a quiet pane when the task's own current no-mistakes run has an `active_steps` row in `running` or `fixing` with a live, non-zombie `agent_pid`, because a validation poll can write nothing to the pane or worktree while its pipeline agent is still running.
That liveness probe uses bounded public `axi status` output (`FM_PIPELINE_AGENT_TIMEOUT`), requires the run branch and code identity to bind to the task worktree under the same current-state rule as `bin/fm-crew-state.sh`, and treats no row, no pid, a dead or zombie pid, a detached worktree, another branch's run, or a stale/diverged head as no evidence.
Every absence of write evidence, including a missing worktree record, a torn-down worktree, a walk that outlives its wall-clock bound on a hung mount, and a failed walk, leaves the existing escalation schedule untouched, so a crew that writes nothing and has no live attributed pipeline agent still escalates exactly as before.
A secondmate's recorded worktree is never probed for write activity, because it is a provisioned firstmate home whose own supervision keeps writing inside it whether or not the mate produces anything, so its panes keep escalating on the unchanged schedule unless another declared-wait rule applies.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the identical reason, escalation count, live-agent and worktree-write deferrals, and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
...
`bin/fm-crew-state.sh <id>` is the cheap current-state read for an actionable heartbeat review: it attributes a no-mistakes run, active or terminal, only when it matches the crew's branch and current code identity, including a pipeline-owned tip this worktree has not fetched, then keeps that run-step authoritative even if the pane has closed.
...
It does share the no-mistakes live-agent deferral, so a quiet validation poll with an attributed live pipeline agent refreshes the stale marker instead of escalating until that evidence disappears.

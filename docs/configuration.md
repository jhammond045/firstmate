...
claude, codex, opencode, pi, pi-signed, grok, kimi, and cursor are empirically verified for crewmate and secondmate launches, while muse and copilot are verified for crewmate and scout launches only; [README requirements](../README.md#requirements) own the set supported for the primary session.
...
copilot is verified for crewmate and scout launches ONLY, and `fm-spawn.sh` refuses it for a secondmate: it has a global hook surface but no documented or verified turn-end event, so a copilot primary's supervision could never be armed.
A copilot launch clears a folder-trust dialog that its autonomy flag does not suppress, and confirms delivery from a real turn record in copilot's own session event log rather than from anything rendered.
copilot substitutes a model the account cannot reach instead of refusing it, so a spawn warns when the model actually running differs from the one requested; [runtime backend verification](verification/runtime-backends.md#github-copilot-cli) owns that evidence.
